...
t_fdf	*create_coordinate(double x, double y, char *s)
{
	t_fdf	*elem;
	int		color;

	color = ft_atoi_base(ft_strchr(s, ',') ? ft_strchr(s, ',') + 1 : "0", 16);
	elem = (t_fdf*)malloc(sizeof(t_fdf));
	elem->x_str = x;
	elem->y_str = y;
	elem->z_str = ft_atoi(s);
	elem->x = x;
	elem->y = y;
	elem->z = ft_atoi(s);
	elem->x_r = x;
	elem->y_r = y;
	elem->z_r = ft_atoi(s);
	elem->color = (color) ? color : 0xFFAABB;
	elem->next = NULL;
	return (elem);
}
...
t_fdf	*writer(char *name, int fd, int y)
{
	...
	while (get_next_line(fd, &name) == 1 && (++y) && !(x = 0))
	{
		...
		if (y == 1 && ++x == 1)
		{
			fdf = create_coordinate((double)x, (double)y, split[x - 1]);
			free(split[x - 1]);
			begin = fdf;
		}
		while (split[++x - 1])
		{
			fdf->next = create_coordinate((double)x, (double)y, split[x - 1]);
			fdf = fdf->next;
			free(split[x - 1]);
		}
		free(split);
